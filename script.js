...
function initStars() {
  ...
  function spawnShootingStar() {
    ...
    const angle = (Math.PI / 180) * (30 + Math.random() * 25); // downward-right diagonal
  ...
  function drawShootingStars() {
    // Rare, brief, white streaks — occasional spawn while at least one is often idle
    ...
    for (const s of shootingStars) {
      ...
      grad.addColorStop(1, `rgba(255,255,255,${Math.max(0, fade * 0.95)})`);
  ...
  requestAnimationFrame(draw);
  if (reduceMotion) draw(0); // single static frame, no shooting stars (respects reduced-motion)
}
...
/* ---------- Random "last download" relative-time text ---------- */
// Purely cosmetic — picks a different realistic relative-time string on every
// page load/refresh. Not tied to any real timestamp, just a fresh random pick each time.
function initLastDownloadTime() {
  const el = document.getElementById('last-download-time');
  if (!el) return;
